...
wbools = [True, False] # Weighted OR Not
...
for model, wbool in list(itertools.product(models, wbools)):
    ...
    if model == 'MURIL':
        ...
        model = AutoModelForSequenceClassification.from_pretrained("simran-kh/muril-cased-temp", num_labels=5)
        model_name = 'MURIL_cased_temp_Malayalam'
    if model == 'XLMR':
    ...
    if model == 'mbertlarge':
        ...
        model = BertForSequenceClassification.from_pretrained('distilbert-base-multilingual-cased', num_labels=5)
        model_name = 'Distilbert_m_base_cased_Malayalam'
    if model == 'XLMR_custom':
        # Using XLMRoberta finetuning Custom Pretrained model, Vocab same => Tokenizer base
        from transformers import XLMRobertaTokenizer, XLMRobertaForSequenceClassification
        tokenizer = XLMRobertaTokenizer.from_pretrained('xlm-roberta-base')
        model = XLMRobertaForSequenceClassification.from_pretrained('../../pretrained_models/XLMroberta_best_pretrained_malayalam/', num_labels=5)
        model_name = 'XLMroberta_custom_pretrained_Malayalam'
    if model == 'mbertbase':
        # Using Multilingual Bert, bert-base-multilingual-cased pretrained
        from transformers import BertTokenizer, BertForSequenceClassification
        tokenizer = BertTokenizer.from_pretrained('bert-base-multilingual-cased')
        model = BertForSequenceClassification.from_pretrained('bert-base-multilingual-cased', num_labels=5)
        model_name = 'Mbert_base_cased_Malayalam'
    if model == 'indic':        
        # Using Indic Bert
        from transformers import AutoTokenizer, AutoModel, AutoModelForSequenceClassification
        tokenizer = AutoTokenizer.from_pretrained("ai4bharat/indic-bert")
        model = AutoModelForSequenceClassification.from_pretrained("ai4bharat/indic-bert", num_labels = 5)
        model_name = 'Indic_bert_Malayalam'
